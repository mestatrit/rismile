...
public class Heartbeat implements RequestCallback {
	...
	private static String jsession = "java.util.UUID.randomUUID().toString()";
	...
	public static void startHeartbeat() {
		...
		hbTimer = new Timer() {
			public void run() {
				try {
					hb_Builder.sendRequest("code=code&jsession="+jsession, response);
	...
	public void onResponseReceived(Request request, Response response) {
		if (response.getStatusCode() != 12029) {
			...
			if (ok != null) {
				...
				NodeList timestamp = customerElement.getElementsByTagName("timestamp");
				if( timestamp != null && timestamp.item(0) != null)
				{
					String tstamp = timestamp.item(0).getFirstChild().getNodeValue();
					GWT.log("Current:"+tstamp, null);
				}
