...
public abstract class Entry implements EntryPoint {
	...
	private void requestLoader() {
		...
		RequestFactory remoteRequest = new RequestFactory("app/config");
		remoteRequest.get(null, null, new RequestCallback() {
			...
			public void onResponseReceived(Request request, Response response) {
				...
				try {
					...
					if( XMLDataParse.getElementNumber(config, "DV1") > 0 ) {
						RuntimeConf.AddConfig("DV1", "true");
					}
					if( XMLDataParse.getElementNumber(config, "ASSERT") > 0 ) {
						RuntimeConf.AddConfig("ASSERT", "true");
					}
					if( XMLDataParse.getElementNumber(config, "FLOW") > 0 ) {
						RuntimeConf.AddConfig("FLOW", "true");
					}
					if( XMLDataParse.getElementNumber(config, "SESSIONFLOW") > 0 ) {
						RuntimeConf.AddConfig("SESSIONFLOW", "true");
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
...
/*	我们试图在这里完成对 google analytics 的跟踪 */
